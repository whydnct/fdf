...
/*   Updated: 2023/05/17 21:54:31 by aperez-m         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	scale_v_map(t_v_map *v_map)
{
	...
	while(i < v_map->rows)
	{
		while(j < v_map->cols)
		{
			*v_map->vertices[i][j]->x *= *s_v_map->pps;
			*v_map->vertices[i][j]->y = *s_v_map->pps;
			j++;
		}
		j = 0;
		i++;
	...
	while(i < v_map->rows)
	{
		while(j < v_map->cols)
		{
			*v_map->vertices[i][j]->x = zoom * \
				(*v_map->vertices[i][j]->x - img_width);
			*v_map->vertices[i][j]->y = zoom * \
				(*v_map->vertices[i][j]->y - img_height);
			j++;
		}
		j = 0;
		i++;
